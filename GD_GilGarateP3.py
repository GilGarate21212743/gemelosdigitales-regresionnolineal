...
Nombre del alumno: Gil Garate Carlos Andres
Número de control: 21212743
Correo institucional: l21212743@tectijuana.edu.mx
...
def plotresults(t, xo, xa, S):
    ...
    plt.rcParams.update({'font.size': 11})
    fig = plt.figure(figsize=(8, 4))
    plt.plot(t, xo, linestyle='none', marker='x', color=[0.20, 0.60, 0.80], label='$x_o(t)$')
    plt.plot(t, xa, linewidth=0.5, markersize=3, color=[0.85, 0.37, 0.27], label='$x_a(t)$ (Model)')
    plt.xlabel('$t$ $[months]$')
    plt.ylabel('$x_i(t)$ $[cells]$')
    plt.xlim(0, 72)
    xticks = np.arange(0, 73, 8)
